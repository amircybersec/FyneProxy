...
type Config struct {
	...
	Health int // 0: healthly, 1: some tests failed, 2: all tests failed
}
...
func main() {
	ProxyApp := app.NewWithID("com.jigsaw.proxyApp")
	if meta := ProxyApp.Metadata(); meta.Name == "" {
		// App not packaged, probably from `go run`.
		meta.Name = "Proxy App"
		app.SetMetadata(meta)
	}
	//ProxyApp.Settings().SetTheme(&appTheme{theme.DefaultTheme()})
